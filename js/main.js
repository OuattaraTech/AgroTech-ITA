...
/* ─────────────────────────────────────────────────
   ...
   Envoi réel via Web3Forms (https://web3forms.com).
   La clé d'accès est dans le champ caché "access_key"
   de l'index.html — pensez à la renseigner.
───────────────────────────────────────────────── */
(function () {
  ...
  form.addEventListener('submit', e => {
    ...
    fetch(ENDPOINT, {
      ...
      .then(data => {
        ...
        success.classList.remove('form-success--error');
        success.textContent = 'Message reçu — nous vous répondrons très prochainement.';
      ...
      .catch(() => {
        // Erreur réseau ou clé invalide : on prévient l'utilisateur
        success.classList.add('form-success--error');
        success.textContent = 'Envoi impossible. Réessayez ou écrivez-nous à contact@agrolab-ita.tech.';
        success.hidden = false;
        success.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
      })
      .finally(() => {
